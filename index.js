...
const server = app.listen(9000, function () {
  console.log("Commander listening on port 9000");
});
  ...
  getPixels(req.file.path, "image/png", function (err, pixels) {
    ...
    for (var i = 0; i < 4000000; i++) {
      ...
      if (VALID_COLORS.indexOf(hex) === -1) {
        res.send(
          `Pixel on ${i % 2000}, ${Math.floor(i / 2000)} has invalid color.`
        );
        fs.unlinkSync(req.file.path);
        return;
      }
...
wsServer.on("connection", (socket) => {
  ...
  console.log(
    `[${new Date().toLocaleString()}] [+] Client connected: ${socket.id}`
  );

  socket.on("close", () => {
    console.log(
      `[${new Date().toLocaleString()}] [-] Client disconnected: ${socket.id}`
    );
  });
...
setInterval(() => {
  ...
  userCount = Array.from(wsServer.clients).filter(
    (c) => c.lastActivity >= threshold
  ).length;
  brandUsage = Array.from(wsServer.clients)
    .filter((c) => c.lastActivity >= threshold)
    .map((c) => c.brand)
